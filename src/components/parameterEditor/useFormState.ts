...
import { MIN_LENGTH_REQUIRED_MSG, REQUIRED_ERROR_MSG, SHOULD_BE_NUMBER_MSG, RESOURCE_ALLOCATION_DUPLICATES_MSG,SHOULD_BE_GREATER_0_MSG,SHOULD_BE_BOOLEAN_MSG } from "./../validationMessages";
...
const useFormState = (consJsonData?: ConsJsonData) => {
    ...
    const constraintsValidationSchema = useMemo(() => (yup.object({
        time_var: yup.number(),
        max_cap: yup.number(),
        max_shift_size: yup.number(),
        max_shift_blocks: yup.number(),
        hours_in_day: yup.number(),
        resource_constraints: yup.array()
            .of(
                yup.object({
